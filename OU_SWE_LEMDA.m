...
K_max = 8; % the range of Fourier modes is [-K_max, K_max]^2
...
sigma_B = 0.01; % noise of the GB modes
sigma_g = 0.01; % noise of the gravity modes
...
L_u = a1; indcmod = []; indfmod = [];
for i = 1: 2: 2 * Dim_Ug + Dim_UB
    absk = sqrt( kk(1, i)^2 + kk(2,i)^2 );
    if (absk>300)
        sigma_gt = sigma_g*(absk^(-5/3))/(3^(-5/3));
        sigma_Bt = sigma_B*(absk^(-5/3))/(3^(-5/3));
    else
        sigma_gt = sigma_g; sigma_Bt = sigma_B; 
    end
    
    if (absk<4.0 && i>2*Dim_Ug)
        indcmod = [indcmod i i+1];
    elseif (i>2*Dim_Ug)
        indfmod = [indfmod i i+1];
    end
    
    if i <= 2 * Dim_Ug
        b1(i,i) = 1 / sqrt(2) * sigma_gt;
        b1(i+1, i+1) = -1i / sqrt(2) * sigma_gt;
        b1(i, i+1) = 1i / sqrt(2) * sigma_gt;
        b1(i+1, i) = 1 / sqrt(2) * sigma_gt;
    else
        b1(i,i) = 1 / sqrt(2) * sigma_Bt;
        b1(i+1, i+1) = -1i / sqrt(2) * sigma_Bt;
        b1(i, i+1) = 1i / sqrt(2) * sigma_Bt;
        b1(i+1, i) = 1 / sqrt(2) * sigma_Bt;
    end
end
indcmod = indcmod - 2 * Dim_Ug;
indfmod = indfmod - 2 * Dim_Ug;
...
%
ckk = kk(:,indcmod); crk = rk(:,indcmod); ca0 = a0(indcmod); ca1 = a1(indcmod,indcmod); cSigma_u = Sigma_u(indcmod,indcmod); 
fkk = kk(:,indfmod); frk = rk(:,indfmod); fa0 = a0(indfmod); fa1 = a1(indfmod,indfmod); fSigma_u = Sigma_u(indfmod,indfmod); 
cDim_U = length(ckk(1,:)); fDim_U = length(fkk(1,:)); 

for i = 2:N  
    u_hat(:,i) = u_hat(:,i-1) + (L_u * u_hat(:,i-1) + a0) * dt + Sigma_u * sqrt(dt) * rd(end-Dim_U+1:end, i);
end
...
for i = 2:10:round(T/dt/100)
    ...
    vc = sqrt(0.5*u.^2 + 0.5*v.^2);
    hold on; contourf(xx,yy,vc,40,'edgecolor','none'); colorbar; %caxis([0, 0.3])
