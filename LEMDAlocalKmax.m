...
% sigv = 0;
% np = 6400; % np is the total number of particles in the; nqq observed
% % 
% maxo = solveParticleModel(domain, sigma_xy, sigv, np, dt, kk, rk, N, u_hat,beta);
% timeLEMDAparticle = toc

%% large scale EuDA part
% OUParaSetting % run to get large scale ocean parameters
...
%%
% fine scale LaDA using local kmax = 4
OUParaSettingFine
nqq = 5; savedn = 5000;
...
%% LaDA in each cell
tic
sigma_xy = 0.001*2*pi/hx; sigma_v = 0.005*2*pi/hx;
...
for jy = 1:ny
    ...
    for jx = 1:nx
        ...
        xa = jx*hx - hx - pi; xb = jx*hx - pi; ya = jy*hy - hy - pi; yb = jy*hy - pi;
        xxl = 2*pi*(xl(indl:indr,:) - xa)/hx - pi; % xi = 2pi(x-a)/(b-a)-pi local cell transform to [-pi, pi]^2
        xxr = 2*pi*(xr(indl:indr,:) - xa)/hx - pi;
        yyl = 2*pi*(yl(indl:indr,:) - ya)/hy - pi;
        yyr = 2*pi*(yr(indl:indr,:) - ya)/hy - pi;
        
        fmu0 = 0.001*randn(fn,1)+0.001i*randn(fn,1);
        fmu0(1:2*nqq) = 0;
        ...
        fb2b = fSigma_u*fSigma_u'*hx/2/pi;
...
toc
save(['./uhat/LEMDAfuhat' num2str(npi,'%02.f') '.mat'],"sfu_post_mean", "sfu_post_cov");
...
for i = 1:4
    subplot(4,2,2*i-1)
    hold on
    indd = 2*nqq+mod(2*(i-1)+1,9); % for kmax = 3
...
%% plots
figure
pnx = 36; pny = 36; h = 2*pi/pnx;
[nxx,nyy] = meshgrid(linspace(-pi+0.5*h,pi-0.5*h,pnx), linspace(-pi+0.5*h,pi-0.5*h,pny));
nx_vec = [reshape(nxx,[],1), reshape(nyy,[],1)]; % becoming a two column matrix

fnx = pnx/nx; fny = pny/ny; fh = 2*pi/fnx;
[fxx,fyy] = meshgrid(linspace(-pi+0.5*fh,pi-0.5*fh,fnx), linspace(-pi+0.5*fh,pi-0.5*fh,fny));
fx_vec = [reshape(fxx,[],1), reshape(fyy,[],1)]; % becoming a two column matrix
...
for jy = 1:ny
    for jx = 1:nx
        ...
        temfvx = (0.5*hx/pi)*exp(1i * fx_vec * fkk) * (utem .* transpose(frk(1,:)));
        temfvy = (0.5*hy/pi)*exp(1i * fx_vec * fkk) * (utem .* transpose(frk(2,:)));
        temfvx = reshape(real(temfvx), fny, fnx);
        temfvy = reshape(real(temfvy), fny, fnx);
        
        fvx((jy-1)*fny+1:jy*fny, (jx-1)*fnx+1:jx*fnx) = temfvx;
        fvy((jy-1)*fny+1:jy*fny, (jx-1)*fnx+1:jx*fnx) = temfvy;
    end
end
subplot(4,4,9)
