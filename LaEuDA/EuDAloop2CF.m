%********************************************************************
%
%  LEMDA: Lagrangian Eulerian Multiscale Data Assimilation (EuDAloopCF Part)
%
...
for npi = 1:length(npset)
    ...
    maxo = solveParticleModelCFj(domain, sigma_xy, sigma_v, np, npi, dt, kk, rk, N, u_hat,beta);
    [npi toc]
end
...
for npi = 1:length(npset)
    ...
    np = npset(npi);
    for j=1:N/savedn
        %file_name = sprintf('/g/data/zv32/seaIceFloeData/cfdata/np%02dtime%03d.mat', npi,j);
        file_name = sprintf('./data/np%02dtime%03d.mat', npi, j);
        load(file_name)
    ...
    timeEuDA = toc
    %save(['/g/data/zv32/seaIceFloeData/cfdata/np' num2str(npi,'%02.f') 'uhat.mat'],"u_post_mean", "u_post_cov");
    save(['./data/np' num2str(npi,'%02.f') 'uhat.mat'],"u_post_mean", "u_post_cov");
...
save('/g/data/zv32/seaIceFloeData/cfdata/ws.mat')
