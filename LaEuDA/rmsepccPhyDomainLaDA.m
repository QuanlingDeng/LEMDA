...
hx = (domain(2) - domain(1))/20;
hy = (domain(4) - domain(3))/20;
...
%      vyda = exp(1i * nx_vec * kk) * (u_post_mean(2*nqq+1:end,j) .* transpose(rk(2,:)));
     vxda = exp(1i * nx_vec * kk) * (ffu_post_mean(2*nqq+1:end,j) .* transpose(rk(1,:))); 
     vyda = exp(1i * nx_vec * kk) * (ffu_post_mean(2*nqq+1:end,j) .* transpose(rk(2,:)));
...
%save(['./err/LaDAnpi' num2str(npi,'%01.f') 'err.mat'], "err")
