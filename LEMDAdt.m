...
sigma_n = 0.05; % noise in the number density discretized ODEs
beta = 1; 
tic
% domain = [-pi pi -pi pi];
...
% sigma_xy = 0.005; % noise in the Lagrangian tracer equations
% np = 8100; nqq = 20; % np is the total number of particles in the; nqq observed
% 
% maxo = solveParticleModel(domain, sigma_xy, np, dt, kk, rk, N, u_hat,beta);
toc
...
for j=1:N
    [npsol(:,:,j), npusol(:,:,j), npvsol(:,:,j)] = ProcEuData(ny, nx, npsol(:,:,j), npusol(:,:,j), npvsol(:,:,j) );
end
...
for i = 2:N
    ...
    nnsol = npsol(:,:,i);
    npsols = reshape(npsol(:,:,i),[],1);
    npusols = reshape(npusol(:,:,i),[],1);
    npvsols = reshape(npvsol(:,:,i),[],1);
    dnn = zeros(2*ndim,1);
    dnn(1:ndim,1) = reshape(npusol(:,:,i) - npusol(:,:,i-1),[],1);   
    dnn(1+ndim:2*ndim,1) = reshape(npvsol(:,:,i) - npvsol(:,:,i-1),[],1);   

    A00 = zeros(2*ndim,1);
    dvv = getDivVV(domain, nx, ny, npusol(:,:,i), npvsol(:,:,i), npusol(:,:,i)./nnsol);
    A00(1:ndim,1) = reshape(dvv,[],1);
    dvv = getDivVV(domain, nx, ny, npusol(:,:,i), npvsol(:,:,i), npvsol(:,:,i)./nnsol);
