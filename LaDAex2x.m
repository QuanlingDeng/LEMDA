...
%OU_SWEex1 % kmax = 6; incompressible flow; only GB modes

sigma_xy = 0.005; % noise in the Lagrangian tracer equations
np = 200; nqq = 80; % np is the total number of particles in the 
...
%maxo = solveParticleModel(domain, sigma_xy, np, dt, kk, rk, N, u_hat,alpha);
toc
...
subplot(2,4,5)
vx = exp(1i * nx_vec * kk) * (u_post_mean(1:Dim_U,ind+1) .* transpose(rk(1,:)));
vy = exp(1i * nx_vec * kk) * (u_post_mean(1:Dim_U,ind+1) .* transpose(rk(2,:)));
vx = reshape(vx, pny, pnx);
vy = reshape(vy, pny, pnx);
quiver(xx, yy, vx, vy, 'linewidth',1.5)
xlim([-pi, pi ])
